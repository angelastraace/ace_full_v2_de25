  ...
  const readContract = async () => {
    ...
    try {
      ...
      if (isConnected && provider) {
        ...
        // Otherwise use a public provider
      ...
      if (ethers.isAddress(result)) {
        ...
        formattedResult = result.map((item) => (typeof item === "bigint" ? item.toString() : item)) // Array
      } else {
      ...
      <CardContent className="space-y-6">
        <Tabs defaultValue="connect">
          ...
          <TabsContent value="contract" className="space-y-4 pt-4">
            ...
            {error && (
              <div className="p-3 text-sm bg-red-900/50 text-red-300 rounded-md border border-red-800">{error}</div>
            )}
